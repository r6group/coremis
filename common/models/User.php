...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function checkHospcode($array_hospcode, $array_username = []){

        if($this->profile === NULL) {
            return False;
        } else {
            $allow_user = false;
            if (count($array_username) > 0) {
                $allow_user = in_array($this->username, $array_username);
            }
            return $allow_user || in_array($this->profile->off_id, $array_hospcode);
        }
    }
