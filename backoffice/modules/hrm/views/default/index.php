...
<?= $hrm_title ? '<h3>' . $hrm_title . '</h3>' : null; ?>
<div class="row">
    <div class="col-sm-8 col-md-9 col-lg-10 people-list">
        <div class="people-options clearfix">
            ...
            <div class="btn-group pull-right people-pager">
                <a href="<?= Url::current(['page' => $pages->page]) ?>" type="button"
                   class="btn btn-default<?= ($pages->page == 0) ? ' disabled' : '' ?>"><i
                        class="fa fa-chevron-left"></i></a>
                <a href="<?= Url::current(['page' => $pages->page + 2]) ?>" type="button"
                   class="btn btn-default<?= ($pages->page + 1 == $pages->pageCount) ? ' disabled' : '' ?>"><i
                        class="fa fa-chevron-right"></i></a>
            </div>
                <span
                    class="people-count pull-right">Showing <strong><?= ($pages->page * $pages->pagesize) + 1 ?>
                        -<?= ($pages->page + 1) * $pages->pagesize ?></strong> of <strong><?= $pages->totalCount ?></strong> users</span>
        </div>
        <!-- people-options -->
        ...
        foreach ($models as $model) {
            ...
            if (!empty($model['avatar_url'])) {
                $avatar = 'http://zone6.cbo.moph.go.th/phi/images/avatars/' . $model['avatar_url'];
            }
                    ...
                    <div class="media">
                        ...
                        <div class="media-body">
                            <h4 class="media-heading">
                                <?= Html::a('' . $model['name'] . ' ' . $model['surname'], ['default/view', 'id' => $model['id'], 'name' => $model['name']]) ?>

                                <span
                                    class="media-usermeta small"><?= ArrayHelper::getValue($model->getPositionArray(), $model->main_pst, '(ไม่ระบุตำแหน่ง)') . ' ' . ArrayHelper::getValue($model->getPostypenameArray(), $model->plevel, ''); ?></span>
                            </h4>

                            <p class="media-usermeta"><i class="glyphicon glyphicon-briefcase"></i>
                                <?= ArrayHelper::getValue($model->getHosArray(27), $model->off_id, 'ไม่ระบุหน่วยงาน') ?>
                            </p>
                        </div>
                    </div>
                    <!-- media -->
