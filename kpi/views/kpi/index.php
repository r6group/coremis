...
<div class="widget">
    <div class="widget-header">
        ...
        <div class="btn-group widget-header-toolbar">
            <div class="control-inline toolbar-item-group">
                <?=$kpi[0]['hdc'] == 1? '(ข้อมูลจาก '. Html::a('HDC <i class="fa fa-external-link"></i>', $hdc_url) .' : '.$hdc_date.')': '<a href="'.Url::toRoute(['kpi-sum/index', 'KpiSumSearch[kpi_id]' => $kpi[0]['id'], 'id' => $kpi[0]['id']]) .'"
    ...
    <div class="widget-content" style="overflow: hidden;">
        ...
        $columns = [
            ...
            ['label' => $lv == "0" ? 'เขต' : 'จังหวัด','noWrap' => true, 'attribute' => 'provname',
                'format'=>'html',
                'value' => function ($model, $key, $index, $widget) use ($next_lv, $id) {
                    if ($next_lv > 2) {
                        return $model['provname'];

                    } else {
                        return $model['provname'] == '' ? '' : Html::a($model['provname'] , ['/kpi/index', 'id'=>$id, 'lv'=>$next_lv, 'z'=> $model['provid']]);
