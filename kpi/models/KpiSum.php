...
{
    const UPLOAD_FOLDER = 'report_attaches';
        ...
        if(in_array($type, ['attach_files','content_file','qwin_q1','qwin_q2','qwin_q3','qwin_q4','rep_q1','rep_q2','rep_q3','rep_q4'])){
            ...
            if(is_array($files)){
                ...
                foreach ($files as $key => $value) {
                    $contents_file .= '<li>'.Html::a('<i class="fa fa-download"></i> '.$value[0]. ' ('.$this->formatBytes($value[1]) .')',['/kpi-sum/download','id'=>$this->id,'file'=>$key,'file_name'=>$value[0]]).'</li>';
                }
                $contents_file .='</ul>';
            }
        }
    ...
    public function listImagesMasonry(){
        ...
        if(is_array($files)){
            ...
            foreach ($files as $key => $value) {
                ...
                $contents_file .= '<a href="/'.$this::UPLOAD_FOLDER.'/'.date('Y-m', strtotime($this->content_date)).'/'.$ref.$key.'">';
                ...
                $contents_file .= '<img src="/'.$this::UPLOAD_FOLDER.'/'.date('Y-m', strtotime($this->content_date)).'/'.$ref.$key.'" class="img-responsive" alt="">';
